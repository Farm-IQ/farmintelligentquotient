...
export class AgentNavbar implements OnInit, OnDestroy {
  ...
  private loadUserProfile(): void {
    this.agentService.getUserProfile().pipe(takeUntil(this.destroy$)).subscribe({
      next: (profile: any) => {
  ...
  private loadNotifications(): void {
    this.agentService.getNotifications().pipe(takeUntil(this.destroy$)).subscribe({
      next: (notifs: any[]) => {
  ...
  toggleNotifications(): void {
    ...
    if (this.showNotifications && this.unreadNotifications > 0) {
      this.agentService.markNotificationsAsRead().pipe(takeUntil(this.destroy$)).subscribe();
    }
  }
