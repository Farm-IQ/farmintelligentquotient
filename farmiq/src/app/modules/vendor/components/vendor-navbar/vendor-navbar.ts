...
@Component({
  ...
  styleUrl: './vendor-navbar.scss'
})
export class VendorNavbar implements OnInit, OnDestroy {
  ...
  private loadUserProfile(): void {
    this.vendorService.getUserProfile().pipe(takeUntil(this.destroy$)).subscribe({
      next: (profile: any) => {
  ...
  private loadNotifications(): void {
    this.vendorService.getNotifications().pipe(takeUntil(this.destroy$)).subscribe({
      next: (notifs: any[]) => {
  ...
  toggleNotifications(): void {
    ...
    if (this.showNotifications && this.unreadNotifications > 0) {
      this.vendorService.markNotificationsAsRead().pipe(takeUntil(this.destroy$)).subscribe();
    }
  }
  ...
  logout(): void {
    this.vendorService.logout().subscribe({
