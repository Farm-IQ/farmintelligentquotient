...
export class FarmiqIdService {
   ...
   * @returns Promise<boolean> true if ID exists, false if not
   */
  private async checkIfIdExists(farmiqId: string): Promise<boolean> {
    try {
      ...
      // Query user_profiles table for this farmiq_id
      const { data, error } = await client
        .from('user_profiles')
        .select('id')
        .eq('farmiq_id', farmiqId)
        .maybeSingle();
      
      // If error, return false (ID doesn't exist)
      if (error) {
        console.warn(`Warning checking FarmIQ ID uniqueness: ${error.message}`);
        return false; // Assume doesn't exist on error
      }
      
      // If data exists, ID is duplicate
      if (data) {
        return true; // ID exists - duplicate
      }
      
      // No data means ID is unique
      return false;
    } catch (err) {
      console.error('Error checking ID existence:', err);
      // On error, assume doesn't exist (safer than rejecting signup)
      return false;
    }
  }
